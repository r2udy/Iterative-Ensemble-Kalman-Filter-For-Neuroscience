# -*- coding: utf-8 -*-
"""
Created on Mon Jun  2 18:33:39 2025
...
#############################################################
# --------- Perform the Ensemble Kalman Filtering --------- #
#############################################################
...
a = np.array([cmro2_lower / cmro2_by_M, 80.0, 80.0])
b = np.array([cmro2_upper / cmro2_by_M, 2.0, 10.0])
enkf.initialize_ensemble(a, b)

# Covariance matrix
Q = np.array([[M_std, 0., 0.],      # Background Covariance Matrix
             [0.,     2., 0.],
             [0.,     0., 10.]])
R = 4.0 * np.eye(obs_dim)              # Observation Covariance Matrix
...
for i, entry in enumerate(uniform_dataset):
    ...
    # # ----------------------+ LSQNonLin +----------------------#
    # obs_ = obs.reshape((20, 20), order='F')
    # analyzer = Po2Analyzer(obs_, M=0.0)
    ...
    state_ensembles[i, :] = enkf.ensemble[0, :]
    ...
    R0 = analyzer.rout
    Rt = analyzer.rout
    generator = MapGenerator(cmro2=mean[0], pvessel=mean[1], Rves=Rves, R0=mean[2], Rt=Rt)
    ...
    ax.set_title('compare', fontsize=18)
    fig.colorbar(surf, ax=ax, shrink=0.5, aspect=10)
    plt.tight_layout()
    plt.show()
    print("\n\n Ensemble Kalman Filter paramaters estimation:")
    print("-"*65)
    print(f"Label         | {'CMRO2'} (umol /cm^3 /min)         | {'Max Pressure'} (mmHg)   | {'R0'} (um)")
    print("-"*65)
    print(f"Means         | {cmro2_mean:0.3f} umol /cm^3 /min           | {mean[1]:0.3f} mmHg               | {mean[2]:0.3f} um")
    print(f"Covariances   | {cmro2_cov:0.3f} umol /cm^3 /min            | {cov[1, 1]:0.3f} mmHg             | {cov[2, 2]:0.3f} um")
    print(f"Std Deviation | {np.sqrt(cmro2_cov):0.3f} umol /cm^3 /min   | {np.sqrt(cov[1, 1]):0.3f} mmHg    | {np.sqrt(cov[2, 2]):0.3f} um")
...
print("\n\nTotal mean: ")
print(f"Means | {np.mean(means, axis=0)[0]*cmro2_by_M:0.3f} umol /cm^3 /min | {np.mean(means, axis=0)[1]:0.3f} mmHg | {np.mean(means, axis=0)[2]:0.3f} um")
std_cmro2 = np.sqrt(np.mean(covariances[:, 0, 0])) * cmro2_by_M
std_p = np.sqrt(np.mean(covariances[:, 1, 1]))
std_r0 = np.sqrt(np.mean(covariances[:, 2, 2]))
print(f"Std Deviation | {std_cmro2:0.3f} umol /cm^3 /min   | {std_p:0.3f} mmHg    | {std_r0:0.3f} um")
