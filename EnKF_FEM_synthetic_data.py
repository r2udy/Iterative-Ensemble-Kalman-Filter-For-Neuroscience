...
obs_var = 2.0**2 # measurement uncertainty
...
for i, cmro2_true in enumerate(cmro2_true_values):
    ...
    cov = 100
    it = 0
    while error.mean() > 2. or np.sqrt(cov)*cmro2_by_M>.15:
        # Form synthetic data to avoid ensemble collapse 
        sigma = 1.5
        obs_perturbated = np.random.normal(obs, sigma)
        ...
        state_ensembles[i, :] = enkf.get_ensemble()
        

        overall_ensemble_track.append(enkf.get_ensemble())

        # -------------- Plot the distributions at each step -------------- #
        # Sample data
        data = np.array(enkf.get_ensemble().T) * cmro2_by_M
        data = data.flatten()  # Flatten in case it's 2D
        ...
        # 1D Histogram plot
        fig, ax = plt.subplots()
        ax.hist(data, bins=30, density=True, alpha=0.6, color='g')  # Histogram with density

        # Mean and standard deviation lines
        ax.axvline(mean_, color='k', linestyle='dashed', linewidth=1.5, label=f'Mean = {mean_:.2f}')
        ax.axvline(mean_ - std_, color='b', linestyle='dotted', linewidth=1.5, label=f'Std = {std_:.2f}')
        ax.axvline(mean_ + std_, color='b', linestyle='dotted', linewidth=1.5)

        # Labels and title
        ax.set_xlabel('U')
        ax.set_ylabel('PDF f(U,t)')
        ...
        print(f"Absolute Mean Error of the EnKF approach: {error.mean()}")
        print(f"The mean is: {cmro2_mean} and the standard deviation is: {np.sqrt(cov)*cmro2_by_M}")
    ...
    print("-"*65)
    print(f"\nCMRO2 Mean: {cmro2_mean}, Rves: {Rves}, R0: {R0}, CMRO2 √(Cov): {np.sqrt(cmro2_cov)}, cmro2 Covariance: {cmro2_cov}\n")
