...
SPACING = 1          # Microns per pixel — must be 1 for this code to work correctly.
...
try:
    ...
    recording = df.iloc[row_to_plot]
    points_x = recording['pointsX'][0]
    points_y = recording['pointsY'][0]
    po2_timeseries = recording['pO2Value']

    # Require a 20x20 record; otherwise mark as bad and exit.
    if np.size(po2_timeseries) != 400:
        print(f"[BAD RECORD] Row {row_to_plot}: pO2Value has size {np.size(po2_timeseries)} (expected 400 for 20x20). Skipping.")
        sys.exit(0)

    po2_timeseries = np.reshape(po2_timeseries, (20, 20))

    plt.figure(figsize=(14, 6))
    plt.imshow(po2_timeseries, cmap='jet')
    plt.title(f"Record: {row_to_plot}, raw data")
    plt.tight_layout()
    plt.show()

    # --- Resampling the Grid ---
    upscale_factor = 10  # this code expects 1 µm per pixel after upsampling
    po2_timeseries_upsampled = np.repeat(po2_timeseries, upscale_factor, axis=0)
    po2_timeseries_upsampled = np.repeat(po2_timeseries_upsampled, upscale_factor, axis=1)

    new_points_x = np.repeat(points_x, upscale_factor)
    new_points_y = np.repeat(points_y, upscale_factor)

    plt.figure(figsize=(14, 6))
    plt.imshow(po2_timeseries_upsampled, cmap='jet')
    plt.title(f"Record: {row_to_plot}, upsampled (x10) raw data")
    plt.tight_layout()
    plt.show()
...
# Grid Setup
# ----------------------------
x = np.arange(0, new_points_x.shape[0], SPACING)
y = np.arange(0, new_points_y.shape[0], SPACING)
X0, Y0 = np.meshgrid(x, y)

# ----------------------------
# Smoothing and Peak Detection
# ----------------------------
Z_smooth = gaussian_filter(po2_timeseries_upsampled.copy(), sigma=SIGMA_SMOOTH)

min_distance_px = int(10 / SPACING)
threshold_abs = (np.max(Z_smooth) + np.min(Z_smooth)) / 2

# Z_smooth.T for real data
...
peak_coords = [(X0[row, col], Y0[row, col]) for row, col in coordinates]

# ----------------------------
# Profile Sampling Utilities
# ----------------------------
def circle_profile(P, X, Y, xc, yc, r_values):
    """Mean pO2 over full circles around (xc, yc)."""
    ...
    for i, r in enumerate(radii):
        ...
        y_idx = np.clip(np.round(y_circle).astype(int), 0, X.shape[0] - 1)
...
def half_sector_contains_other_centers(coords, i, r_um, theta0, allowed_indices, tol_frac=TOLERANCE_FRAC, eps=0.0):
    ...
    Returns:
        (contains: bool, offending_index_or_None)
    """
    xi, yi = coords[i]
...
def run_analysis(P, X, Y, peak_coords):
    """
    Two-pass logic with per-radius clipping only (no accumulated/overall clipping).
    """
    ...
    for i, (xc, yc) in enumerate(peak_coords):
        ...
        d1 = np.diff(means1)
        inv = -d1
        # peaks_all, _ = find_peaks(inv, prominence=0.015)
        ...
        if SHOW_DETECTION_PLOTS:
            ...
            axs[1].axhline(-R_ART_MIN_PEAK, color='orange', linestyle='--',
                           label=f"threshold: dpO2/dr ≤ {-R_ART_MIN_PEAK:.2f}")
            if len(peaks_all) > 0:
                ok_mask = inv[peaks_all] >= R_ART_MIN_PEAK
                if np.any(~ok_mask):
                    axs[1].scatter(radii1[peaks_all[~ok_mask]], d1[peaks_all[~ok_mask]],
                                   s=50, color='red', label="minima (weak)")
                if np.any(ok_mask):
                    axs[1].scatter(radii1[peaks_all[ok_mask]], d1[peaks_all[ok_mask]],
                                   s=50, color='green', label="minima (≥ threshold)")
                if chosen_peak_idx is not None and inv[chosen_peak_idx] >= R_ART_MIN_PEAK:
                    axs[1].scatter([radii1[chosen_peak_idx]], [d1[chosen_peak_idx]],
                                   s=120, marker='*', color='gold', edgecolor='black', zorder=5,
                                   label="chosen minimum")
            axs[1].set_title(
        ...
        if r_art_radius is None:
            ...
            print(f"[R_art] {tag_i}: accepted at r≈{r_art_radius:.0f} µm "
                  f"(peak height {peak_height:.3f} ≥ {R_ART_MIN_PEAK:.3f}).")
            R_art_v.append([i, r_art_radius])

    # ---------- Containment filter (only among RETAINED centers, with tolerance) ----------
    ...
    if len(kept_indices) >= 2:
        for i in list(kept_indices):
            ...
            for j in kept_indices:
                ...
                # containment uses effective radius R_art * (1 + tol)
                if d <= ri_eff:
    ...
    # ---------- PASS 2: R_tissue among retained centers ----------
    R_tissue_v = []
    if len(kept_indices) == 1:
        ...
        radii2, means2, per_clip = full_circle_profile_for_tissue(
            P, X, Y, xc, yc, r_values_2
        )
        d2 = np.diff(means2)            # first derivative ~ dpO2/dr
        d2_2 = np.diff(d2)              # second derivative ~ d²pO2/dr²

        try:
            cand = np.where(np.abs(d2) <= DERIV_THRESH)[0]
            if len(cand) == 0:
                d2min, d2max = float(np.min(d2)), float(np.max(d2))
                print(f"[R_tissue] {tag_i}: no radius where |dpO2/dr| <= {DERIV_THRESH:.2e} "
                      f"(range [{d2min:.3e}, {d2max:.3e}]) → NOT DETECTED.")
                R_tissue_idx = None
            else:
                ...
                for idx in cand:
                    ...
                    idx2 = int(np.clip(idx - 1, 0, len(d2_2) - 1))
                    sec_val = float(d2_2[idx2]) if len(d2_2) > 0 else 0.0
        ...
        if SHOW_DETECTION_PLOTS:
            ...
            axs[1].plot(radii2[:-1], d2, marker='x', color='black')
            if R_tissue_idx is not None:
        ...
        for i in kept_indices:
            ...
            for j in kept_indices:
                ...
                d2 = np.diff(means2)     # first derivative
                d2_2 = np.diff(d2)       # second derivative
...
def main():
    ...
    # 2D maps with detected maxima
    fig, axs = plt.subplots(1, 2, figsize=(14, 6))
    axs[0].imshow(po2_timeseries_upsampled, cmap='jet')
    axs[0].set_title("pO2 With Detected Local Maxima")
    axs[1].imshow(Z_smooth, cmap='jet')
    axs[1].set_title("Smoothed pO2 With Detected Local Maxima")

    pe_outline = [pe.Stroke(linewidth=3, foreground='black'), pe.Normal()]
    for ax in axs:
        for x_peak, y_peak in coordinates:
            ax.scatter(x_peak, y_peak, marker='x', s=160, c='white', linewidths=2.5, path_effects=pe_outline)

    plt.tight_layout()
    plt.show()

    # 3D surface with local maxima
    fig3 = plt.figure(figsize=(8, 6))
    ax3 = fig3.add_subplot(111, projection='3d')
    ax3.plot_surface(X0, Y0, Z_smooth, cmap='jet', edgecolor='none')
    for x_peak, y_peak in coordinates:
        z_peak = Z_smooth[int(y_peak), int(x_peak)] + 20
        ax3.scatter(x_peak, y_peak, z_peak, color='red', edgecolor='black', s=120, marker='^', linewidth=1.5)
    ax3.set_title("Smoothed pO2 with Detected Local Maxima")
    ax3.set_xlabel("x (µm)"); ax3.set_ylabel("y (µm)"); ax3.set_zlabel("pO2 (mmHg)")
    plt.tight_layout()
    plt.show()
    ...
    if len(peak_coords) >= 1:
        R_art_v, R_tissue_v = run_analysis(Z_smooth, X0, Y0, coordinates)
    else:
        print(f"Expected at least 1 peaks for analysis, but found {len(peak_coords)}.")
        return
    ...
    ax_final.set_title("Smoothed pO2 with Estimated R_art and R_tissue")
    ...
    # Helper to keep labels inside the image
    h, w = Z_smooth.shape
    PAD = 8
    ...
    # R_tissue overlay (full circle for single-center kept, half-circle for multi-center kept)
    for (i_j, R_tissue_idx) in R_tissue_v:
        i, j = i_j
        if R_tissue_idx is None:
            continue
        if not (0 <= R_tissue_idx < len(r_values_2)):
            print(f"[Overlay] {center_tag(i, coordinates)} (j={None if j is None else j+1}): "
        ...
        if j is None:
            ...
            ax_final.text(lx, ly, f"R_tissue ≈ {r_um:.0f} µm",
                          color=color_i, fontsize=8, va='center', path_effects=pe_outline)
        else:
            xj, yj = coordinates[j]
            ...
            ax_final.text(lx, ly, f"R_tissue ≈ {r_um:.0f} µm",
                          color=color_i, fontsize=8, ha='center', va='bottom', path_effects=pe_outline)

    # Legend for centers that have at least one valid R_tissue
    from matplotlib.lines import Line2D
    valid_r_tissue_centers = set()
    for (i_j, R_tissue_idx) in R_tissue_v:
        i, j = i_j
        if R_tissue_idx is not None:
            valid_r_tissue_centers.add(i)

    legend_handles = []
    for i in sorted(valid_r_tissue_centers):
        x_peak, y_peak = coordinates[i]
        legend_handles.append(
            Line2D([0], [0],
                   color=tab10_colors[i % len(tab10_colors)], lw=2,
                   label=f"Center {i + 1} ({x_peak:.0f}, {y_peak:.0f})")
        )
    if legend_handles:
        ax_final.legend(handles=legend_handles, loc='upper left', fontsize=9, frameon=True)
