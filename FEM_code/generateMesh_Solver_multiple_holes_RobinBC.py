...
class HoleGeometry:
    ...
    def __init__(self, center, radius_ves, Pves, radius_0=None, cmro2=None, marker=3, bc_type="dirichlet", permeability=0.0):
        self.center = center
...
class DiffusionSolver:
    ...
    def setup_problem(self, params, holes):
        ...
        self.setup_neumann_bcs([holes[0]])
        ...
        for hole in holes:
            marker = 100 + hole.marker
            ds_hole = ufl.ds(subdomain_data=self.facet_tags, subdomain_id=marker)
            g = fem.Constant(self.domain, ScalarType(-0.0))
            self.F += g * self.v * ds_hole

        # Add Robin BC terms
        self.setup_robin_bcs(holes)
        for term in self.robin_terms:
            self.F += term
    ...
    def setup_dirichlet_bcs(self, holes):
        ...
        for hole in holes:
            if hole.bc_type != "dirichlet":
                continue # Skip non-Dirichlet holes
    ...
    def setup_robin_bcs(self, holes):
        """Set up Robin boundary conditions on circles around holes"""
        self.robin_terms = []

        for hole in holes:
            if hole.bc_type != "robin":
                continue # Skip non-Robin holes
            ...
            # LHS: k * u * v * ds
            robin_lhs = k * self.uh * self.v * ds_hole

            # RHS: k * Pb * v * ds
            robin_rhs = k * Pb * self.v * ds_hole
            
            self.robin_terms.append(robin_lhs)
            self.robin_terms.append(-robin_rhs)
...
def main():
    ...
    params = SolverParameters(filename="square_holes_no_neumann_R0_120", cmro2_background=0.0)
