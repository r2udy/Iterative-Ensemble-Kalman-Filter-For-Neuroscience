...
from FEM_code.generateMesh_Solver_multiple_MM import DiffusionSolver, SolverParameters, HoleGeometry
...
file_id_saving = "Test_stats/MM_71"  # ID for saving the data
...
R0_mean_initial = 100.
R0_var_initial = 2.**2
R0_var = 1.e-1**2 # prior uncertainty of capillary-free space radius
...
# Prior associated with Km: pO₂ at which tissue consumes oxygen at 50% of its maximum rate
Km_mean_initial = 20.0
Km_var_initial = 1.0**2
Km_var = 1.0**2  # prior uncertainty of Km
...
state_dim   = 4  # [cmro2, R0, pvessel, Km]
obs_dim     = 400
n_ensembles = 50
...
a = np.array([cmro2_mean_inital / cmro2_by_M,       R0_mean_initial, pvessel_mean_initial, Km_mean_initial])
b = np.array([cmro2_var_initial / (cmro2_by_M)**2,  R0_var_initial,  pvessel_var_initial, Km_var_initial])
...
B   = np.array([[M_var,    0.0,        0.0,         0.0],
                [0.0,      R0_var,     0.0,         0.0],
                [0.0,   0.0,           pvessel_var, 0.0],
                [0.0,   0.0,           0.0,         Km_var]])   # Background covariance matrix
...
for i, entry in enumerate(uniform_dataset):
    ...
    params = SolverParameters(filename="square_holes", Km = Km_mean, cmro2_background=0.0)
    ...
    Km_est_enkf.append(Km_mean)
    Km_cov_est_enkf.append(Km_cov)
    ...
    state_ensembles[3, i, :] = enkf.ensemble[3, :]  # Km ensembles
...
Km_est_enkf = np.array(Km_est_enkf)
Km_cov_est_enkf = np.array(Km_cov_est_enkf)
...
for i in range(numBoxes):
    ...
    P.plot(x, y, 'r.', alpha=0.2)
P.xlabel('$PO_{2}$ Map ID')
P.ylabel('State values Km (mmHg)')
P.title('EnKF Km State Estimation with Uncertainty')
P.grid(True)
P.savefig(path + 'enkf_Km_state_estimation.png', dpi=300, bbox_inches='tight')
# P.show()
...
x_obs = np.arange(1, numBoxes + 1) 
cov_track = np.array([np.std(array) for array in data])
ax.plot(x_obs, cov_track, '-o', color='blue', label='Uncertainty in R0 estimation (StD)')
...
x_obs = np.arange(1, numBoxes + 1) 
cov_track = np.array([np.std(array) for array in data])
ax.plot(x_obs, cov_track, '-o', color='blue', label='Uncertainty in Pvessel estimation (StD)')
