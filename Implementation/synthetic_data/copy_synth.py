...
cmro2_lower_array, cmro2_upper_array = np.array([0.5, 1.0, 1.5]) , np.array([1.5, 2.0, 2.5])
cmro2_var = (cmro2_upper - cmro2_lower)**2 / 12
M_var = cmro2_var / cmro2_by_M **2 / 10000 # model uncertainty scaled
obs_var_high = 5.**2
...
cmro2_true_values = np.linspace(1., 2., 3)
...
for i, cmro2_true in enumerate(cmro2_true_values):
    ...
    profile_main = generator.pO2_array.reshape((n, n), order='F')
    ...
    profile_secondary = generator2.pO2_array.reshape((n, n), order='F')
    profile_secondary[profile_secondary < 0] = 0
    profile_secondary = gaussian_filter(profile_secondary.flatten(), sigma=2.).reshape((n, n), order='F')

    # Add noise to the generated data
    true_obs = profile_main 
    sigma2 = 2.0
    obs_perturbated = np.random.normal(true_obs.flatten(), scale=sigma2)
    obs_perturbated_array = obs_perturbated.reshape((n, n), order='F')
    ...
    #     obs_var_low=obs_var_low
    # )
    R = obs_var_high * np.eye(obs_dim) # Observation covariance matrix
    ...
    ax.plot_surface(X, Y, obs.reshape((n, n), order='F'), cmap='viridis', alpha=0.3, edgecolor='none')
    ax.set_xlabel('X (nm)')
    ax.set_ylabel('Y (nm)')
    ax.set_zlabel('pO2 (mmHg)')
    plt.colorbar(sc, ax=ax, shrink=0.3, aspect=10, label='pO2 (mmHg)')
...
path = "/Users/ruudybayonne/Desktop/Stanford_Biology/PROJECT_OxyDiff/Python_code/Data/EnKF_plots/synthetic_data/constant/"
