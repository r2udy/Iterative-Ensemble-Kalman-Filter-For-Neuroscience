...
------------------------------------
  ...
  N_ENS_BOOT    = 50  ensemble members
  ITER_MAX_BOOT = 10  iterations
...
SAVE_PATH  = os.path.join(ROOT, "Data/EnKF_plots/sensitivity_analysis_bootstrap/")
...
N_ENS_BOOT      = 50    # ensemble members in bootstrap
ITER_MAX_BOOT   = 10    # iterations in bootstrap

# Physical validity threshold
BIAS_FLAG_THRESHOLD = 1.0   # umol/cm3/min -- flag if |bias| exceeds this
...
def _make_enkf(obs_std, cmro2_std_phys, R0_std, pvessel_std,
    ...
    enkf = EnKF(STATE_DIM, OBS_DIM, _n, dyn, seed=seed)
...
def run_bootstrap_sweep(sweep_name, sweep_values):
    """
    ...
    """
    keys  = ["cmro2_mean","cmro2_std","signed_bias","bias","pO2_abs","spread"]
    n     = len(sweep_values)
